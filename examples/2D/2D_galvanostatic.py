...
def solve_problem(
    ...
    mesh = dolfinx.mesh.create_rectangle(
        MPI.COMM_WORLD, np.array([[0, 0], [W, H]]), [Nx, Ny], cell_type=cell_type
    )
    ...
    _Kc = dolfinx.fem.Constant(mesh, Kc)
    _eps = dolfinx.fem.Constant(mesh, epsilon)
    _u_in = dolfinx.fem.Constant(mesh, u_in)
    _p_out = dolfinx.fem.Constant(mesh, p_out)
    ...
    values[dolfinx.mesh.locate_entities_boundary(mesh, mesh.topology.dim - 1, inlet)] = 4
    values[dolfinx.mesh.locate_entities_boundary(mesh, mesh.topology.dim - 1, outlet)] = 5
    ...
    # -------------------------------------------------------------------------
    # Solve pressure independently first using Darcy equation
    # -------------------------------------------------------------------------
    # Darcy coefficient (Kozeny–Carman)
    beta = (_df**2 / (_Kc * _mu)) * (_eps**3) / ((1.0 - _eps) ** 2)

    el = basix.ufl.element("Lagrange", mesh.basix_cell(), degree)

    Qp = dolfinx.fem.functionspace(mesh, el)
    p_trial = ufl.TrialFunction(Qp)
    q_p = ufl.TestFunction(Qp)

    a_p = ufl.inner(beta * ufl.grad(p_trial), ufl.grad(q_p)) * dx
    L_p = _u_in * q_p * ds(4)

    outlet_facets = ft.find(5).astype(np.int32)
    p_bc_dofs = dolfinx.fem.locate_dofs_topological(Qp, mesh.topology.dim - 1, outlet_facets)
    bc_p = dolfinx.fem.dirichletbc(_p_out, p_bc_dofs, Qp)

    pressure_problem = dolfinx.fem.petsc.LinearProblem(
        a_p,
        L_p,
        bcs=[bc_p],
    ...
    print("---------pressure / velocity solve---------")
    p_h = pressure_problem.solve()
    assert isinstance(p_h, dolfinx.fem.Function)
    p_h.name = "p [Pa]"

    # Fixed Darcy velocity used later in the concentration equations
    v_darcy = -beta * ufl.grad(p_h)

    # With constant Darcy parameters and uniform boundary conditions,
    # the velocity is uniform, so we can prescribe it directly as the inlet velocity.
    ...
    u_mag = ufl.sqrt(ufl.dot(v_darcy, v_darcy) + eps_speed**2)

    # Reynolds number Re = rho |u| d_f / mu
    Re = _rho * u_mag * _df / _mu
    ...
    c_V3_s = ((1.0 + A / (k_mR + eps_c)) * c_V3_eff + (A / (k_mO + eps_c)) * c_V2_eff) / den
    c_V2_s = ((1.0 + B / (k_mO + eps_c)) * c_V2_eff + (B / (k_mR + eps_c)) * c_V3_eff) / den
    ...
    residual += _i_app / (_z_H * _F) * v_c_H * ds(3)

    # Outlet species fluxes
    # Convective outflow through outlet boundary
    residual += (
        ufl.dot(v_darcy, n_vec) * c_V2 * v_c_V2 * ds(5)
        + ufl.dot(v_darcy, n_vec) * c_V3 * v_c_V3 * ds(5)
        + ufl.dot(v_darcy, n_vec) * c_H * v_c_H * ds(5)
        + ufl.dot(v_darcy, n_vec) * c_HSO4 * v_c_HSO4 * ds(5)
    )

    # Dirichlet Boundary condition:
    # Constant inlet concentration:
    inlet_facets = ft.find(4).astype(np.int32)
    ...
    Vv = dolfinx.fem.functionspace(mesh, v_el)

    # The Darcy velocity is time-independent in this model.
    v_h = dolfinx.fem.Function(Vv)
    v_h.name = "v [m/s]"

    v_expr = dolfinx.fem.Expression(v_darcy, Vv.element.interpolation_points)
    v_h.interpolate(v_expr)
    ...
    eta_summary_expr = dolfinx.fem.Expression(eta, Vs.element.interpolation_points)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-H", type=np.float64, default=1e-2, help="Height of electrode (M)")
    ...
    parser.add_argument("--p_out", type=np.float64, default=3.0e5, help="Outlet pressure (Pa)")

    parser.add_argument(
        "--output_file", type=str, default="2D_transient", help="Name of the output files"
    )

    parser.add_argument(
        "--outdir", type=Path, default=Path().cwd() / "output" / "2D", help="Output directory"
    )

    parser.add_argument("--snes_atol", type=float, default=1e-10, help="SNES absolute tolerance")
